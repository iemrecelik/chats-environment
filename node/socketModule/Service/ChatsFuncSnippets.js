...
const client = require(__defined.baseUrl + '/config/RedisConn.js');
...
module.exports = class ChatsFuncSnippets{
    ...
    async updateOnlUsrTime({io = null, socket = null}){
        
        await client.hgetallAsync('chat:online').then((loginTimeList) => {

            if(io) io.of('chats').emit('updateOnlUsrTime', loginTimeList);
            else if(socket) socket.emit('updateOnlUsrTime', loginTimeList);
        });
    }
